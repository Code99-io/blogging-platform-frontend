...
export interface LikeListViewType {
  search?: string;
  onEdit: (id: number) => void;
  onDelete: (id: number) => void;
}

const LikeListView: React.FC<LikeListViewType> = ({
  search,
  onEdit,
  onDelete,
}) => {
  ...
  const { data, refetch, isFetching, isError, error } = useLikes(
    page,
    pageSize,
    search,
  );

  const columns: DataGridProps<LikeListItemType>['columns'] = [
    {
      key: 'id',
      name: 'Id',
      width: 80,
